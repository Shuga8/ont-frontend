...
const InfoToast = ({ message, isActive }) => {
  <div
    ...
    } items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow border-t-4 primary-t-green-800`}
    role="alert"
  >
    <div className="inline-flex items-center justify-center flex-shrink-0 w-8 h-8  rounded-lg bg-primary-800 text-primary-200">
      <HiMiniInformationCircle />
      <span className="sr-only">Success icon</span>
    </div>
    <div className="ms-3 text-sm font-normal text-primary-500">{message}</div>
    <button
      type="button"
